...
class MainAction extends DashAction {
  ...
  public function performAction($dashboard){
    ...
    if(!($dashboard->getDashboardUser()->isAdministrator() || $dashboard->getDashboardUser()->isEditor())){
      $query = 'UPDATE '.$cm->getPostsTableString().' SET title=:title, introduction=:introduction, banner=:banner, content=:content, date=:date, category=:category, tags=:tags, friendly_name=:new_friendly_name, status=:status WHERE friendly_name=:friendly_name AND poster=:poster';
    } else {
      $query = 'UPDATE '.$cm->getPostsTableString().' SET title=:title, introduction=:introduction, banner=:banner, content=:content, date=:date, category=:category, tags=:tags, friendly_name=:new_friendly_name, status=:status WHERE friendly_name=:friendly_name';
    }
