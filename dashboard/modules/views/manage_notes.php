...
class MainView extends DashView{
  ...
  public function generateView($dashboard){
    if(!isset($_GET['data'])){
      ...
      $query = 'SELECT * FROM '.$cm->getNotesTableString() . ' WHERE user_id = :user_id';
      ...
      $form->addHtml($cm->generateBlogPagination($page_number, 8, DashboardLinks::EDIT_CONTENT_VIEW.'?page_number=', $stmt->rowCount()));
      ...
      //Add the navigation
      $form->addHtml($cm->generateBlogPagination($page_number, 8, DashboardLinks::MANAGE_NOTES_VIEW.'?page_number=', $stmt->rowCount()));
